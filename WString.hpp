...
namespace myAlgorithm {
    class WString {
        ...
        friend std::wostream &operator<<(std::wostream &wos, const WString &ws);

        friend std::wistream &operator>>(std::wistream &wis, WString &ws);
    ...
    std::wostream &operator<<(std::wostream &wos, const WString &ws) {
        for (WString::size_type i = 0; i < ws._size; ++i)
            wos << ws.s[i];
        return wos;
    }

    std::wistream &operator>>(std::wistream &wis, WString &ws) {
        ws.clear();
        wchar_t ch;
        ch = getwchar();
        while (iswspace(ch))
